...
var pathSum = function (root, targetSum) {
  ...
  if (!root) return 0;
  let flag = findPath(root, targetSum);
  let sum1 = pathSum(root.left, targetSum);
  let sum2 = pathSum(root.right, targetSum);
  return flag + sum1 + sum2;
};

const findPath = function (root, sum) {
  if (!root) return 0;
  let flag = root.val === sum ? 1 : 0;
  let leftSum = findPath(root.left, sum - root.val);
  let rightSum = findPath(root.right, sum - root.val);
